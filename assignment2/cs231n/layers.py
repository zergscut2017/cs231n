...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        # The implementation of algorithm 1 (arxiv: 1502.03167v3)
        sample_mean = np.mean(x, axis=0)  # mini-batch mean
        sample_var = np.var(x, axis=0)    # mini-batch standard variance
        x_normalized = (x-sample_mean) / np.sqrt(sample_var + eps)  # normalize
        out = gamma*x_normalized + beta   # scale and shift (Yi is the out)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    #out[i, f, j, k] = np.sum(x_padded[i(ith image),(all color dimension), j*s:HH+j*s (x axis) , k*s:WW+k*s(y axis)] * w[f]) + b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
    ...
    dx = dx_padded[:, :, pad:pad+H, pad:pad+W] # Unpad
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
